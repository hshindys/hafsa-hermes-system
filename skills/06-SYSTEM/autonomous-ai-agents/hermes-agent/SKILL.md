...
"second brain," read the vault to build/update `SOUL.md`. Covers identity
extraction, relationship precision, health reminders, and the write pipeline:
`skill_view(name="hermes-agent", file_path="references/vault-to-soul-pipeline.md")`.
...
When `hermes auth add xai-oauth --no-browser` can't open a browser (server,
container, SSH), use PTY mode to capture the OAuth URL, SSH tunnels for remote
hosts, and `--manual-paste` for browser-only consoles:
`skill_view(name="hermes-agent", file_path="references/grok-oauth-headless.md")`

### Voice cloning with MOSS-TTS-Nano (100M params)

Give an AI persona its own cloned voice using reference audio. Produces OGG
Opus clips deliverable as Telegram voice bubbles. Runs on CPU in ~3-5s per
clip. Full setup (conda env, model download, generator script, emotion presets,
pitfalls):
`skill_view(name="hermes-agent", file_path="references/moss-tts-nano-voice-cloning.md")`.

### Discord gateway conflict debugging

When Discord slash commands are received but responses never arrive, or the
gateway restart counter keeps growing — check for dual-gateway conflicts (two
profiles running the same bot token), and fix Discord `require_mention` /
`auto_thread` config that prevents server-channel responses:
`skill_view(name="hermes-agent", file_path="references/discord-gateway-conflict-debug.md")`.
...
```bash
hermes profile list                        # show all profiles + gateway status
hermes profile create NAME                 # new profile
hermes profile use NAME                    # set sticky default
hermes -p NAME gateway start               # start a named profile's gateway
```

**Platform migration** — when switching Discord/Telegram from `default` to a named profile (token-lock conflict, takeover sequence, diagnostic steps): `references/profile-platform-migration.md`.
...
| MOSS-TTS-Nano (local) | None (voice cloning from reference audio) | Free |

For MOSS voice cloning setup (local model, emotion presets, OGG output):
`skill_view(name="hermes-agent", file_path="references/moss-tts-nano-voice-cloning.md")`

### Plugin-free Obsidian vault MCP (23 tools)

When the user has a large Obsidian vault and wants full CRUD + FTS5 search + links + frontmatter + daily notes without Docker or Obsidian plugins:
`skill_view(name="hermes-agent", file_path="references/vault-cortex-plugin-free-mcp.md")`

Full script reference (paths, usage, training data format, directory layout):
`skill_view(name="hermes-agent", file_path="references/voice-cloning-scripts.md")`

### World Cup / Sports Tournament Tracking
When the user is following a sports tournament and wants match results updated
in their vault:
`skill_view(name="hermes-agent", file_path="references/world-cup-tracking-pattern.md")`

Key steps: search for results → update main tracker file → update group standings → update daily templates → update top scorers.

Umbrella voice-cloning skill (user-facing, includes Discord troubleshooting):
`skill_view(name="voice-cloning-tts")`
...
When the user wants a unified dashboard for all AI tools (Dreaming, Mission Control, Cost Tracking, Pantheon skill visualization, Document Hub), set up an Agentic OS:

`skill_view(name="agentic-os-dashboard")` for the full dashboard setup guide.

Key components:
- **Dreaming:** Nightly analysis + morning brief (cron job)
- **Mission Control:** Goal management with visual progress tracking
- **Cost Tracking:** Live AI spend monitoring by platform/hour/day
- **Pantheon:** Visual skill/persona inventory
- **Document Hub:** Unified document management

NOT for: single-tool usage, basic chat, simple task execution.
...
#### Discord gateway restart loop (repeated disconnect/reconnect)

**Symptoms:** Gateway logs show `✓ discord connected` followed by `discord disconnected` every few minutes. `systemctl --user status` shows a high restart counter (e.g., `restart counter is at 18`).

**Diagnosis:**
```bash
# Check restart counter in systemd status
systemctl --user status hermes-gateway-<profile> --no-pager | grep "restart counter"

# Check disconnect pattern in logs
grep -i "discord.*disconnect\|discord.*error\|discord.*fail" ~/.hermes/profiles/<profile>/logs/gateway.log | tail -20

# Check if it's a token conflict (two profiles using same bot token)
grep -i "token already in use" ~/.hermes/profiles/<profile>/logs/gateway.log
```

**Common causes:**
- **Token conflict**: Two profiles (e.g., `default` and `hafsa`) both configured with the same `DISCORD_BOT_TOKEN`. The second gateway fails to bind and the first gets kicked. Fix: stop the other profile's gateway, or use different tokens.
- **Network instability**: Discord WebSocket connection drops due to network issues. The gateway reconnects automatically but if it happens too frequently, it looks like a loop.
- **Discord API rate limits**: Too many connection attempts in a short period.

**Fix sequence:**
1. Stop ALL gateways: `systemctl --user stop hermes-gateway-<profile>` (for each running profile)
2. Check which profiles are running: `hermes profile list`
3. Start only the profile you want: `hermes -p <profile> gateway start`
4. Verify: `systemctl --user status hermes-gateway-<profile> --no-pager`

**⚠️ Cannot restart from inside the gateway process:**
`hermes gateway restart` and `hermes -p <profile> gateway restart` will refuse with "Refusing to restart the gateway from inside the gateway process" if called from a tool running inside the running gateway. Always tell the user to run the restart from their terminal.
