...
+ كتابة الرواية + مكالمة الأسرة + أذكار الصباح والمساء والنوم والاستيقاظ)
وبيحطه في ملاحظة اليوم بالخزنة.
...
def _hermes_config_dir() -> Path:
    """Locate the Hermes config dir cross-platform.

    Linux/macOS: ~/.hermes/config
    Windows:     ~/AppData/Local/hermes/config
    The .hermes dir wins if it exists (portable installs).
    """
    home = Path.home()
    dot = home / ".hermes" / "config"
    if dot.exists():
        return dot
    local = home / "AppData" / "Local" / "hermes" / "config"
    local.mkdir(parents=True, exist_ok=True)
    return local


CONFIG_PATH = _hermes_config_dir() / "routine.toml"

# Fallback config so the script runs even before routine.toml exists.
# Edit routine.toml (same dir) to override any of these values.
DEFAULT_CONFIG = {
    "location": {
        "latitude": 30.0444,       # Cairo (change to your city)
        "longitude": 31.2357,
        "calculation_method": 5,   # Egyptian General Authority of Survey
        "timezone": "Africa/Cairo",
    },
    "cache": {"dir": "~/AppData/Local/hermes/cache/daily_routine"},
    "schedule": {
        "quran_minutes_after_fajr": 20,
        "quran_duration_minutes": 30,
        "morning_adhkar_minutes_after_fajr": 35,
        "asma_minutes_after_fajr": 35,
        "writing_minutes_after_asr": 30,
        "writing_duration_minutes": 60,
        "evening_adhkar_minutes_after_maghrib": 20,
        "family_call_minutes_after_maghrib": 60,
        "sleep_adhkar_minutes_before_sleep": 15,
        "sleep_time": "22:30",
    },
    "vault": {
        "section_header": "## 📅 الروتين اليومي",
        "path": "C:/Users/hshin/OneDrive/Documents/Hafsa",
...
def load_config() -> dict:
    if not CONFIG_PATH.exists():
        print(f"⚠️ مفيش routine.toml في {CONFIG_PATH} — بستخدم الإعدادات الافتراضية.")
        return DEFAULT_CONFIG
    try:
        with open(CONFIG_PATH, "rb") as f:
            return tomllib.load(f)
    except Exception as e:
        print(f"⚠️ مشكلة في قراءة routine.toml ({e}) — بستخدم الإعدادات الافتراضية.")
        return DEFAULT_CONFIG
...
def hhmm(timing: str) -> str:
    # الـ API بيرجع "04:11 (EEST)" — بناخد أول 5 حروف بس
    return timing[:5]
...
def format_multi(label_title: str, items: list[str]) -> str:
    """يحول قائمة أذكار إلى نص منسق للجدول."""
    lines = [label_title]
    for item in items:
        lines.append(f"- {item}")
    return "\n".join(lines)
...
def build_schedule(cfg: dict, timings: dict, day_index: int) -> tuple[list[tuple[str, str]], str]:
    ...
    morning_adhkar_time = add_minutes(fajr, s.get("morning_adhkar_minutes_after_fajr", s.get("asma_minutes_after_fajr", 35)))
